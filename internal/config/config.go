...
func Load() (config *Config, err error) {
	config = &Config{}
	path, ext := os.LookupEnv(ENV_PATH); if !ext {
		path = DEFAULT_CONFIG_PATH
	}

	err = hclsimple.DecodeFile(path,nil,config); if err != nil {
...
func isValid(config *Config) (bool,string) {
	...
	if config.Terrapak.Hostname == "" {
		return false,"[ERROR] - hostname is required is required in terrapak.hcl"
	}
